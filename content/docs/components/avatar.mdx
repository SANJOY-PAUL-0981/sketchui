...
| `name` | string | - | User name used for generating initials fallback. |
| `size` | AvatarSize | `"md"` | Controls the size of the avatar. |
| `variant` | AvatarVariant | `"gray"` | Controls the avatar color theme. |
| `bgColor` | string | - | Overrides the default avatar background color. |
| `borderColor` | string | `"#111"` | Border color of the avatar. |
| `initials` | string | - | Custom initials displayed when using initials fallback. |
| `showInitials` | boolean | `false` | Displays initials instead of an image. |
| `status` | AvatarStatus | - | Status indicator displayed on the avatar. |
| `showStatus` | boolean | `false` | Enables the status badge. |
| `rotate` | number | `0` | Rotates the avatar in degrees. |
| `hoverWiggle` | boolean | `false` | Adds a playful wiggle animation on hover. |
| `className` | string | - | Additional classes applied to the avatar wrapper. |
| `imageClassName` | string | - | Additional classes applied to the image element. |
| `fallbackClassName` | string | - | Additional classes applied to the fallback content. |
| `roughOptions` | RoughAvatarOptions | - | Custom RoughJS rendering configuration. |
